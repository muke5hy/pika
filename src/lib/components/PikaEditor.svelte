<script lang="ts">
    ...
    async function copyImage() {
        ...
        if (isSafari) {
            ...
            try {
                const permission = await (navigator as any).permissions?.query?.({ name: "clipboard-write" });
...
>
    <div class="relative flex flex-col-reverse w-full lg:flex-row-reverse max-w-[1600px] mx-auto">
            ...
            <div class={cls(
                ...
                <div class="relative flex flex-row flex-wrap items-start justify-start space-y-5 lg:items-start lg:flex-col lg:space-y-4">
                    ...
                    {#if blob?.src && !showCropper}
                        <div class="w-full">
                            <button
                                on:click={() => { showCropper = true; initializeCropper(); }}
                                class="w-full flex items-center justify-center px-3 py-2 text-sm font-semibold text-white bg-purple-600 rounded-lg shadow hover:bg-purple-700 duration-200">
                                <span class="w-4 h-4 mr-2" innerHTML={CropIcon}></span>
                                Crop Image
                            </button>
                        </div>
                    {/if}
                    ...
                    <div class="w-full space-y-2">
                        ...
                        <div class="flex items-center gap-2">
                            <input
                                ...
                                placeholder="Width"
                                class="w-full px-2 py-1 text-sm border border-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-400"
                            />
                    ...
                    {#if showCropper}
                        <div class="w-full space-y-2">
                            <div class="flex items-center gap-2">
                                <button
                                    on:click={applyCrop}
                                    class="flex-1 px-3 py-2 text-sm font-semibold text-white bg-pink-600 rounded-lg shadow hover:bg-pink-700 duration-200">
                                    Done
                                </button>
                                <button
                                    on:click={resetCrop}
                                    class="px-3 py-2 text-sm font-semibold text-pink-600 bg-pink-100 rounded-lg shadow hover:bg-pink-200 duration-200">
                                    Reset
                                </button>
                                <button
                                    on:click={cancelCrop}
                                    class="px-3 py-2 text-sm font-semibold text-gray-600 bg-gray-200 rounded-lg shadow hover:bg-gray-300 duration-200">
                                    Cancel
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div>
                            <select bind:value={options.browserBar}
                                class="px-2 py-1 border border-gray-500 rounded-lg shadow-lg appearance-none cursor-pointer opacity-80 hover:opacity-100"
                                on:change={(e) => options = {...options, browserBar: (e.target as HTMLSelectElement).value }}>
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div>
                            <select bind:value={options.padding}
                                class="px-2 py-1 border border-gray-500 rounded-lg shadow-lg appearance-none cursor-pointer opacity-80 hover:opacity-100"
                                on:change={(e) => options = {...options, padding: (e.target as HTMLSelectElement).value }}>
                    ...
                    <div>
                        <div class="relative flex items-center pb-2 text-sm font-semibold dark:text-white">
                            Background
                            <div class="relative">
                                <div on:click={toggleBG} class="flex items-center px-2 ml-2 border border-gray-400 rounded-lg cursor-pointer bg-white/70 opacity-70 hover:opacity-100 dark:bg-pink-900/80 dark:border-red-600 dark:text-gray-300">
                            ...
                            {#if bgPicker}
                                <!-- backdrop to close -->
                                <div class="fixed inset-0 w-full h-full bg-transparent" on:click={() => bgPicker = false}></div>
                                ...
                                <div class={cls(
                                    ...
                                    <div class="absolute top-[5%] right-[5%] opacity-50 cursor-pointer hover:opacity-100 z-10" on:click={() => bgPicker = false}>✕</div>
                                        ...
                                        <div class="flex items-center">
                                            <div class="relative group">
                                                <input id="startColorPicker" type="color"
                                                    ...
                                                    on:input={(e) => options = {...options, customTheme:{...(options.customTheme||{}), colorStart:(e.target as HTMLInputElement).value}}} />
                                        ...
                                        <div class="flex items-center">
                                            <div class="relative group">
                                                <input id="endColorPicker" type="color"
                                                    ...
                                                    on:input={(e) => options = {...options, customTheme:{...(options.customTheme||{}), colorEnd:(e.target as HTMLInputElement).value}}} />
                        ...
                        <div class="grid flex-wrap grid-cols-6 mt-1 gap-x-4 gap-y-2">
                            {#each [
                                ...
                                <div class={"cursor-pointer shadow dark:shadow-black/20 shadow-gray-500/20 w-8 h-8 rounded-full " + theme}
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div>
                            <select bind:value={options.roundedWrapper}
                                class="px-2 py-1 border border-gray-500 rounded-lg shadow-lg appearance-none cursor-pointer opacity-80 hover:opacity-100"
                                on:change={(e) => options = {...options, roundedWrapper: (e.target as HTMLSelectElement).value }}>
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div>
                            <select bind:value={options.rounded}
                                class="px-2 py-1 border border-gray-500 rounded-lg shadow-lg appearance-none cursor-pointer opacity-80 hover:opacity-100"
                                on:change={(e) => options = {...options, rounded: (e.target as HTMLSelectElement).value }}>
                                <option value="rounded-none">None</option>
                                <option value="rounded-lg">Small</option>
                                <option value="rounded-xl">Medium</option>
                                <option value="rounded-3xl">Large</option>
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div>
                            <select bind:value={options.position}
                                class="px-2 py-1 border border-gray-500 rounded-lg shadow-lg appearance-none cursor-pointer opacity-80 hover:opacity-100"
                                on:change={(e) => options = {...options, position: (e.target as HTMLSelectElement).value }}>
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div>
                            <select bind:value={options.shadow}
                                class="px-2 py-1 border border-gray-500 rounded-lg shadow-lg appearance-none cursor-pointer opacity-80 hover:opacity-100"
                                on:change={(e) => options = {...options, shadow: (e.target as HTMLSelectElement).value }}>
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div>
                            <input type="checkbox" checked={options?.noise || false} class="text-xl" on:change={() => options = {...options, noise: !options?.noise}} />
                        </div>
                    </div>

                    <div class="flex items-center justify-between w-full">
                        <div class="flex items-center justify-center px-4 py-2 hover:scale-[1.03] duration-200 text-base lg:text-lg font-semibold text-pink-600 bg-pink-200 rounded-lg shadow cursor-pointer border border-pink-600 w-full"
                            on:click={copyImage} title="Use Ctrl/Cmd + C to copy the image">
                            <span class="w-6 h-6 mr-2" innerHTML={ClipboardIcon}></span>
                            Copy
                        </div>

                        <div class="flex items-center justify-center px-4 py-2 hover:scale-[1.03] duration-200 text-base lg:text-lg font-semibold bg-pink-600/90 dark:bg-pink-600/90 text-pink-200 rounded-lg shadow cursor-pointer border border-pink-600 w-full ml-4"
                    ...
                    <div on:click={clearBlob} class="flex items-center justify-center w-full px-3 py-1 mx-auto mt-4 text-sm text-pink-400 rounded-lg cursor-pointer">
        ...
        <div class="w-full lg:w-[calc(100%-350px)] py-5 lg:p-10 lg:pl-0 flex flex-col-reverse lg:flex-col items-center justify-center overflow-y-auto">
            {#if showCropper}
                <!-- Inline Cropper View -->
                <div class="w-full max-w-4xl my-5">
                    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden">
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden">
                                {#if originalImageSrc}
                                    <img bind:this={cropperImageElement} src={originalImageSrc} alt="Crop preview" class="max-w-full max-h-[60vh]" />
                                {/if}
                            </div>
                        </div>
                    </div>
            ...
            {:else if blob?.src}
                <!-- Preview Display -->
                <div class={cls(options?.roundedWrapper, "overflow-hidden shadow-xl duration-200 ease-in-out relative my-5")}>
                    <div
                        bind:this={wrapperRef}
                        style={options?.customTheme ? `background: linear-gradient(45deg, ${options.customTheme?.colorStart || "transparent"}, ${options.customTheme?.colorEnd || "transparent"})` : ""}
                        class={cls(
                    ...
                    >
                        ...
                        {#if options?.noise}
                            <div style="background-image: url('/noise.svg')" class={cls("absolute inset-0 w-full h-full bg-repeat opacity-[0.15]", options?.rounded, options.browserBar !== "hidden" ? "rounded-t-none" : "")}></div>
                        {/if}

                        <img src={blob.src}
                            on:load={onImageLoad}
                            class={cls("relative z-10 transition-all duration-200 ease-in-out", options?.shadow, options?.rounded, getImageRadius(), options?.browserBar == "hidden" ? "" : "rounded-t-none")}
                            style={blob?.w ? `width: ${blob.w / (typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1)}px` : ""}
                            alt="Preview" />
                    </div>
                </div>
            {:else}
